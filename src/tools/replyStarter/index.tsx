...
function ReplyStarter() {
  ...
  const { copied: copiedIndex, copy } = useCopyFeedback<number>();
  ...
  const replies = output ? parseNumberedList(output) : [];
        ...
        <ul className="reply-list">
          {replies.map((reply, index) => (
            <li key={index} className="reply-item">
              <p>{reply}</p>
              <button type="button" className="copy-button" onClick={() => copy(index, reply)}>
